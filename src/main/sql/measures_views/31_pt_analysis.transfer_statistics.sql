...
RUNS AS(
	SELECT RUN_NAME,
		TRANSFER_STOP_GROUP,
		COUNT(TRANSFER_STOP_GROUP)*10 TRANSFERS
	FROM TRANSFERS
...
SELECT
	R1.*,
	(R1.TRANSFERS-R1.TRANSFERS)/R1.TRANSFERS::float TRANSFERS_CHANGE
FROM RUNS R1
LEFT JOIN RUNS R2
ON R1.TRANSFER_STOP_GROUP = R2.TRANSFER_STOP_GROUP
WHERE R2.RUN_NAME = 'bc'
