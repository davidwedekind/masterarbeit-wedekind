...
WITH BOEB_ESSL AS (
	SELECT RUN_NAME,
		...
		COUNT(TRIP_ID) / SUM(COUNT(TRIP_ID)) OVER (PARTITION BY RUN_NAME, REL_BOEBL_ESSL) AS SWITCHES_REL
...
FOCUS_AREAS AS (
	SELECT RUN_NAME,
		...
		TO_M_MODE,
		COUNT(TRIP_ID) SWITCHES_ABS,
		COUNT(TRIP_ID) / SUM(COUNT(TRIP_ID)) OVER (PARTITION BY RUN_NAME, REL_FOCUS_AREAS) AS SWITCHES_REL
